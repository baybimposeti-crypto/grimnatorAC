...
public class AdvancedTriggerBotCheck extends Check {
    ...
    private void checkReactionTime(Player player, TriggerBotData data) {
        ...
        if (reactionTime < 50) {
            ...
            if (data.getSuspiciousReactions() >= 3) {
                flagOrShadow(player, "suspiciously fast reaction time (" + reactionTime + "ms)");
    ...
    private void checkAimPrecision(Player player, Entity target, TriggerBotData data) {
        ...
        if (angle < 1.0) {
            ...
            if (data.getPerfectAims() >= 5) {
                flagOrShadow(player, "suspiciously perfect aim (angle: " + String.format("%.2f", angle) + "°)");
        ...
        if (recentAttacks.length >= 3) {
            ...
            if (variance < 1000) { // Less than 1ms variance
                ...
                if (data.getConsistentPatterns() >= 3) {
                    flagOrShadow(player, "suspiciously consistent attack pattern (variance: " + variance + "ms)");
    ...
    private void checkTargetTracking(Player player, Entity target, TriggerBotData data) {
        ...
        if (distance >= 2.5 && distance <= 3.5) { // Optimal PvP distance
            ...
            if (data.getOptimalDistanceAttacks() >= 10) {
                ...
                if (ratio > 0.8) {
                    flagOrShadow(player, "suspicious target tracking (optimal distance ratio: " + String.format("%.2f", ratio) + ")");
    ...
    private void flagOrShadow(Player player, String details) {
        boolean shadow = plugin.getConfigManager().getCheckConfig("combat")
                .getConfig().getBoolean("advancedtriggerbot.shadow-mode", true);
        if (shadow) {
            debug("[shadow] " + player.getName() + " " + details);
            return;
        }
        flag(player, details);
    }
