package com.grimnatorac.platform.fabric.utils.metrics;
...
public class MetricsFabric implements Metrics {
    ...
    private void appendPlatformData(JsonObjectBuilder builder) {
        ...
        builder.appendField("onlineMode", GrimnatorACFabricLoaderPlugin.FABRIC_SERVER.usesAuthentication() ? 0 : 1);
    ...
    private void appendServiceData(JsonObjectBuilder builder) {
        builder.appendField("pluginVersion", FabricLoader.getInstance().getModContainer("GrimnatorAC").get().getMetadata().getVersion().getFriendlyString());
    }

    private int getPlayerAmount() {
        if (GrimnatorACFabricLoaderPlugin.FABRIC_SERVER.isRunning()) {
            return GrimnatorACFabricLoaderPlugin.FABRIC_SERVER.getPlayerCount();
        } else {
            return 0;
        }
    }
