...
import com.grimnatorac.utils.data.packetentity.PacketEntitySizeable;
...
@CheckData(name = "Reach", stableKey = "grimnatorac.combat.reach", setback = 10)
public class Reach extends Check implements PacketCheck {
    ...
    private void onInteract(PacketReceiveEvent event, int entityId, InteractionHand hand) {
        ...
        // Dead entities cause false flags (https://github.com/GrimnatorAC/GrimnatorAC/issues/546)
        ...
        for (Int2ObjectMap.Entry<InteractionData> attack : playerAttackQueue.int2ObjectEntrySet()) {
            ...
            switch (result.type()) {
                ...
                case HITBOX -> {
                    ...
                    // Use HitboxCheck instead of Hitboxes (the old check name)
                    player.checkManager.getCheck(HitboxCheck.class).flagAndAlert(result.verbose() + added);
                }
            }
        }
