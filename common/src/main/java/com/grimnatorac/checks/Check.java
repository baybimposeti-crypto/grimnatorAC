...
public class Check extends GrimProcessor implements AbstractCheck {
    ...
    public boolean shouldModifyPackets() {
        return isEnabled
                ...
                && !exemptPermission
                && !isUkkaExempt(); // Ukka verified users exempt from ALL checks
    }

    /**
     * Checks if the player is a verified Ukka user and should be exempt from all checks.
     * This method provides centralized Ukka exemption logic for all checks.
     *
     * @return true if player is verified Ukka user, false otherwise
     */
    protected boolean isUkkaExempt() {
        try {
            // Query the exemption registry for platform-specific exemptions (e.g., Ukka)
            return com.grimnatorac.checks.exempt.ExemptionRegistry.isExempt(player.user.getUUID());
        } catch (Throwable t) {
            // Silently return false to avoid breaking the check
            return false;
        }
    }
    ...
    public final void updatePermissions() {
        ...
        exemptPermission = player.platformPlayer.hasPermission("grimnatorac.exempt." + id);
        noSetbackPermission = player.platformPlayer.hasPermission("grimnatorac.nosetback." + id);
        noModifyPacketPermission = player.platformPlayer.hasPermission("grimnatorac.nomodifypacket." + id);
    }
    ...
    public final boolean flag(String verbose) {
        if (player.disableGrim || (experimental && !player.isExperimentalChecks()) || exemptPermission || isUkkaExempt())
            return false; // Avoid calling event if disabled or Ukka exempt
